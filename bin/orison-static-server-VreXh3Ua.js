...
const DOCTYPE_PATTERN = /^\s*<!DOCTYPE html>\s*/i;
function html(strings, ...values) {
    const firstChunk = strings[0] ?? '';
    const doctypeMatch = firstChunk.match(DOCTYPE_PATTERN);
    if (!doctypeMatch) {
        return lit.html(strings, ...values);
    }
    const nextStrings = [...strings];
    nextStrings[0] = firstChunk.replace(DOCTYPE_PATTERN, '');
    nextStrings.raw = [...strings.raw];
    nextStrings.raw[0] = strings.raw[0]?.replace(DOCTYPE_PATTERN, '') ?? '';
    return {
        doctype: '<!DOCTYPE html>',
        template: lit.html(nextStrings, ...values)
    };
}
function isOrisonTemplateResult(value) {
    return Boolean(value) && typeof value === 'object' && 'doctype' in value && 'template' in value;
}
...
class OrisonProject {
    ...
    async build() {
        ...
        return {
            ...
            pages: outputs.filter(output => output.contentType === 'text/html').map(output => output.routePath)
        };
    }
    async renderRequest(routePath) {
        const site = await this.inspectSite();
        const normalized = normalizeRequestedRoute(routePath, this.options.fragmentName);
        const publicOutput = this.getPublicOutputForRoute(site.tree, normalized.routePath);
        if (publicOutput) {
            return {
    ...
    async copyStaticDirectory() {
        ...
        await fs.cp(this.staticRoot, this.options.outputRoot, { force: true, recursive: true });
    }
    async createContext(root, directory, page, routePath, fragmentHtml, loaderCache) {
        ...
        const pageContext = {
            ...
            loaders: loaderMap,
            local: directory,
            mdFile: async (filePath) => unsafeHtml_js.unsafeHTML(this.markdown.render(await fs.readFile(this.resolveProjectFilePath(page, filePath), 'utf8'))),
    ...
    async evaluateHtmlTemplate(sourcePath, context) {
        ...
        return templateFactory(context, html);
    }
    getPublicOutputForRoute(directory, routePath) {
        if (directory.path === normalizeDirectoryPathFromRoute(routePath) && directory.localData.public) {
    ...
    async loadLoaders(loaderCache) {
        ...
        const fileLoaders = (await exists(loadersDirectory)) ? await walkFiles(loadersDirectory) : [];
        const loaderEntries = [
    ...
    async readDirectoryTree(relativePath, parent) {
        ...
        const localData = (await exists(localDataPath)) ? JSON.parse(await fs.readFile(localDataPath, 'utf8')) : {};
    ...
    async collectPages(directory) {
        const entries = await fs.readdir(directory.sourcePath, { withFileTypes: true });
    ...
    async renderListPage(root, page, routePath, fragmentOnly, servingSingleSlug) {
        ...
        const slug = servingSingleSlug ? getSlugFromListRoute(page, routePath) : undefined;
        ...
        for (const item of items) {
            ...
            const fullHtml = fragmentOnly ? fragmentHtml : await this.wrapWithLayout(root, page, itemContext, fragmentHtml);
            outputs.push({
                body: fullHtml,
                contentType: 'text/html',
    ...
    getDirectRoutes(page) {
        ...
        if (page.name === 'index') {
            ...
            return [`/${toPosix(page.directory.relativePath)}`, `/${toPosix(page.directory.relativePath)}.html`];
        }
        const prefix = page.directory.relativePath ? `/${toPosix(page.directory.relativePath)}/` : '/';
...
function getSlugFromListRoute(page, routePath) {
    ...
    const pageSegments = page.directory.relativePath.split(path.sep).filter(Boolean);
...
async function renderUnknown(value) {
    ...
    if (isOrisonTemplateResult(value)) {
        return `${value.doctype}\n${await renderResult_js.collectResult(ssr.renderThunked(value.template))}`;
    }
...
function resolveOptions(options) {
    ...
    const sourceRoot = requireFromPackage('node:fs').existsSync(path.join(rootPath, options.srcDirectory ?? 'src', options.pagesDirectory ?? 'pages'))
        ...
        : rootPath;
    return {
        buildDir: options.buildDir ? path.resolve(rootPath, options.buildDir) : path.join(rootPath, 'docs'),
        ...
        protectedFileNames: options.protectedFileNames ?? ['pages', 'partials', 'loaders', 'static'],
        rootPath,
        sourceRoot,
...
class OrisonServer {
    ...
    constructor(options = {}) {
        ...
        this.app.get('*', async (request, response) => {
            ...
            response.status(rendered.statusCode).type(rendered.contentType).send(rendered.content);
        });
    }
    async start() {
...
exports.html = html;
//# sourceMappingURL=orison-static-server-VreXh3Ua.js.map
