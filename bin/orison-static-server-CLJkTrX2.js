...
class OrisonProject {
    ...
    async rewriteImportSpecifier(importerPath, specifier) {
        ...
        if (path.basename(resolved) === 'contentful.js') {
            return path.join(this.options.rootPath, 'src', 'fallback-contentful.ts');
        }
        return specifier;
    }
    async writeCompatibilityModule(filePath) {
        ...
        const compatibilityPath = path.join(this.options.rootPath, '.orison-cache', path.relative(this.options.rootPath, filePath));
    ...
    async rewriteModuleImports(filePath, source) {
        ...
        const requireMatches = [...transformed.matchAll(/(require\(\s*['"])([^'"]+)(['"]\s*\))/g)];
...
//# sourceMappingURL=orison-static-server-CLJkTrX2.js.map
